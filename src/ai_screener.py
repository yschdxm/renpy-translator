...
_TOOLS = [
    ...
    {"type": "function", "function": {
        "name": "search_code",
        "description": "在游戏全部源码中搜索子串，返回匹配行（文件:行号:内容，最多 20 条）。用于查清一个字符串在哪里被使用、是否会被显示给玩家。",
        "parameters": {"type": "object", "properties": {
...
只输出 JSON 数组，不要输出任何其他文字：[{{"id": 1, "keep": true, "confident": true}}, ...]"""
...
class AIScreener:
    ...
    def __init__(self, translator, game_root: str, logger=None):
        ...
        self.game_sub = self.game_root / 'game'
        self.logger = logger
    ...
    def screen_all(self, candidates: list, progress: dict):
        ...
        """
        try:
            self._coarse_screen(candidates, progress)
            self._refine_screen(candidates, progress)
        finally:
    ...
    def _refine_screen(self, candidates: list, progress: dict):
        ...
        # 失败直接上抛，不降级
        for verdicts in self._pool.map(self._refine_batch, batches):
            ...
            progress['done'] = min(progress['done'] + self.REFINE_BATCH,
                                   len(uncertain))

    def _refine_batch(self, batch: list) -> dict:
        """单批 agentic 精审：多轮 tool 循环，返回 {候选索引: (keep, reason)}"""
        id_map = {n: idx for n, (idx, _) in enumerate(batch)}
        items = [{'id': n, 'text': c.text, 'hint': c.hint, 'kind': c.kind,

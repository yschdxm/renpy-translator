# 🎮 Ren'Py 游戏翻译工具

一个基于 NiceGUI 的 Ren'Py 游戏汉化工具，支持 AI 翻译和手动翻译，让游戏汉化更简单。

## ✨ 功能特点

### 📂 项目管理
- 创建翻译项目，绑定游戏目录
- 自动解包 `.rpa` 文件
- 自动反编译 `.rpyc` 文件
- 使用 Ren'Py SDK 生成标准翻译文件
- 项目打包导出 / 导入
- 顶栏快速切换项目

### 👤 人名翻译
- 自动提取游戏角色名
- 支持 AI 批量翻译人名
- 支持手动编辑人名翻译
- 人名词典自动保存

### 🧠 人物分析
- 分析角色台词，生成人物特征
- 包含性格、说话风格、翻译建议等
- 翻译时自动注入角色特征

### 🔤 字符串翻译
- 翻译菜单选项、按钮等 UI 文字
- 支持搜索和筛选
- 支持批量翻译

### 💬 对话翻译
- 翻译游戏对话和旁白
- 支持搜索、筛选、按角色过滤
- 支持上下文显示
- AI 翻译时自动注入角色特征

### 📦 游戏导出
- 一键导出翻译后的游戏
- 自动配置中文字体
- 自动添加语言选择界面
- 导出目录：`projects/项目名/output/`

## 🚀 快速开始

### 安装

```bash
# 克隆项目
git clone https://github.com/yschdxm/renpy-translator.git
cd renpy-translator

# 安装依赖
uv sync
```

### 准备字体

将一款开源中文字体放入 `fonts/` 目录：

```bash
mkdir fonts
# 从 https://fonts.google.com/noto/specimen/Noto+Sans+SC 下载
# 将 .otf 或 .ttf 文件放入 fonts/ 目录
```

### 配置 Ren'Py SDK

1. 下载对应平台的 [Ren'Py SDK](https://www.renpy.org/latest.html)
2. 解压到项目目录（如 `renpy-8.5.3-sdk`）
3. 在工具的"模型配置"页面设置 SDK 路径

**跨平台兼容**：SDK 在不同平台完全兼容：
- Windows: 自动使用 `renpy.exe`
- macOS/Linux: 自动使用 `renpy.sh`

### 配置 AI 模型

1. 在"模型配置"页面添加 AI 模型
2. 支持 OpenAI 兼容接口（如 DeepSeek、Claude 等）
3. 设置 API 地址、Key、模型名称

### 启动

```bash
uv run python run.py
```

访问 http://localhost:7860

## 📖 使用流程

界面采用左侧导航栏 + 右侧内容区的专业布局：

```
Header: 🎮 Ren'Py Translator │ [切换项目]          进度: x/x │ 队列: 空闲
├── 左侧导航                    ├── 右侧内容区
│   ├── 📂 项目管理             │   └── 当前面板内容
│   ├── 👤 人名翻译             │
│   ├── 🧠 人物分析             │
│   ├── 🔤 字符串翻译           │
│   ├── 💬 对话翻译             │
│   ├── 📦 导出游戏             │
│   └── ⚙  模型配置             │
```

### 1. 创建项目
- 点击侧边栏「项目管理」
- 点击「新建项目」，填写项目名、游戏目录、选择 AI 模型
- 项目自动解包资源文件并生成翻译模板

### 2. 人名翻译
- 点击侧边栏「人名翻译」
- 点击「翻译全部人名」或逐个翻译
- 检查并修正翻译结果

### 3. 人物分析
- 点击侧边栏「人物分析」
- 点击「分析所有角色」
- 查看角色特征和翻译建议

### 4. 字符串翻译
- 点击侧边栏「字符串翻译」
- 翻译菜单选项、按钮等 UI 文字

### 5. 对话翻译
- 点击侧边栏「对话翻译」
- 使用搜索和筛选功能
- 点击「AI翻译」或「全部翻译」

### 6. 导出游戏
- 点击侧边栏「导出游戏」
- 点击「开始导出」
- 导出目录：`projects/项目名/output/`

### 切换项目
- 使用顶栏的项目下拉选择器快速切换
- 或在「项目管理」面板点击项目的「打开」按钮

## 📁 项目结构

```
renpy-translator/
├── src/
│   ├── main.py                # NiceGUI 主入口
│   ├── logger.py              # 统一日志系统
│   ├── database.py            # SQLite 数据库层
│   ├── translation_service.py # 统一翻译调度服务
│   ├── translator.py          # AI 翻译器
│   ├── renpy_parser.py        # Ren'Py 脚本解析器
│   ├── rpa_extractor.py       # RPA 文件解包器
│   ├── sdk_manager.py         # Ren'Py SDK 管理器
│   ├── project_manager.py     # 项目管理器
│   ├── config_manager.py      # 配置管理器
│   ├── components/            # 通用 UI 组件
│   │   ├── paginated_table.py # 分页表格
│   │   ├── progress_panel.py  # 进度面板
│   │   └── log_panel.py       # 日志面板
│   └── panels/                # 功能面板
│       ├── project_panel.py   # 项目管理
│       ├── name_panel.py      # 人名翻译
│       ├── analysis_panel.py  # 人物分析
│       ├── text_panel.py      # 文本翻译（字符串+对话共用）
│       ├── export_panel.py    # 游戏导出
│       └── config_panel.py    # 模型配置
├── projects/                  # 项目目录
│   └── 项目名/
│       ├── project.db         # SQLite 项目数据库
│       ├── game/              # 解包后的游戏文件
│       └── output/            # 导出目录
├── exports/                   # 项目打包导出（.zip）
├── config/                    # 配置目录
│   └── models.json            # AI 模型配置
├── fonts/                     # 中文字体目录
├── run.py                     # 启动脚本
└── pyproject.toml             # 项目配置
```

## 🔧 技术栈

- **前端**: [NiceGUI](https://nicegui.io/) (Vue.js + Quasar)
- **后端**: Python 3.12+
- **AI**: OpenAI 兼容接口
- **SDK**: Ren'Py SDK

## ⚠️ 注意事项

1. **请确保有游戏汉化授权**
2. 翻译前建议备份游戏文件
3. 人名词典有助于保持翻译一致性
4. 人物分析可以提高翻译质量

## 📝 翻译提示词

### 人名翻译
- 简洁的提示词
- 只返回中文名
- 自动过滤占位符

### 字符串翻译
- 说明可能的内容类型（菜单、按钮、提示等）
- 符合中文游戏用语习惯

### 对话翻译
- 信达雅的翻译原则
- 自动注入角色特征
- 支持上下文理解

## 🐛 问题反馈

如遇到问题，请在 GitHub 提交 Issue。

---

## ⚖️ 免责声明

### 1. 工具用途

本工具（Ren'Py 游戏翻译工具）仅供**学习、研究和个人使用**。本工具的设计目的是帮助用户翻译**自己拥有合法权利**的游戏作品，或**已获得原作者明确授权**的游戏作品。

### 2. 用户责任

使用本工具的用户应：

- **遵守当地法律法规**：确保您的使用行为符合所在国家和地区的法律法规
- **尊重知识产权**：仅翻译您拥有合法权利或已获授权的游戏作品
- **获取合法授权**：在翻译任何游戏之前，请确保已获得原作者或版权持有人的明确授权
- **承担使用责任**：用户对使用本工具产生的一切后果承担全部责任

### 3. 法律合规

请注意，不同国家和地区对以下内容有不同的法律规定：

- **版权法**：翻译权属于著作权的一部分，未经授权翻译可能构成侵权
- **内容法规**：各国对数字内容的传播有不同的规定
- **数字版权**：DMCA等法律对数字内容有特殊规定
- **出口管制**：某些内容可能受出口管制法规限制

**用户有责任了解并遵守所在地区的相关法律法规。**

### 4. 开发者免责

本工具的开发者：

- **不生产、不存储、不传播**任何游戏内容
- **不鼓励、不支持任何形式的侵权行为**
- **不对用户的使用行为承担任何法律责任**
- **不对因使用本工具造成的任何直接或间接损失承担责任**
- **保留随时修改、更新或终止本工具的权利**

### 5. 版权声明

- Ren'Py 引擎版权归 [Tom Rothamel](https://www.renpy.org/) 所有
- 本工具仅为辅助翻译工具，不拥有任何游戏的版权
- 翻译后的游戏版权归原作者所有
- 用户应尊重原作者的知识产权和劳动成果

### 6. 禁止行为

严禁将本工具用于：

- ❌ 未经授权翻译受版权保护的作品
- ❌ 分发未经授权的翻译作品
- ❌ 侵犯他人知识产权
- ❌ 传播违法内容
- ❌ 任何违法行为

### 7. 免责条款

**本工具按"现状"提供，不作任何明示或暗示的保证。在任何情况下，开发者均不对以下情况承担责任：**

- 任何直接、间接、附带、特殊、惩罚性或后果性损害
- 因违反当地法律而产生的任何后果
- 因侵犯版权而产生的任何索赔

### 8. 使用即表示同意

使用本工具即表示您：

- 已阅读、理解并同意本免责声明的所有条款
- 确认您所在地区允许使用本工具
- 承诺遵守所有适用的法律法规

### 9. 法律适用

本免责声明的解释和执行应适用开发者所在地区的法律。任何争议应通过友好协商解决。

---

**最后提醒：**

- 🎮 请尊重游戏开发者的劳动成果，支持正版游戏
- ⚖️ 请遵守当地法律法规，尊重知识产权
- 💡 本工具仅供学习研究，请勿用于商业用途
